"""
chart_items.py — the pyqtgraph primitives that actually paint the charts.

These are the custom QGraphicsObject/ImageItem/delegate classes the panels put
inside their plots: the footprint clusters, the delta footer, the heatmap
candles, the DOM depth bars. Nothing here knows about MainWindow or the model
registry -- each takes a model duck-typed at draw time.

("items" is pyqtgraph's word for these; this module used to be called that, which
